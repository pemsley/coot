/*
     pygl/ppmutil.cc: CCP4MG Molecular Graphics Program
...
#define LIBPNG_SOLIBRARY_SPEC "libpng12.so.0"
#define LIBGIF_SOLIBRARY_SPEC "libungif.so.4"
#define LIBJPG_SOLIBRARY_SPEC "libjpeg.so.62"
#define LIBTIFF_SOLIBRARY_SPEC "libtiff.so.3"
#define LIBXPM_SOLIBRARY_SPEC "libXpm.so.4"
...
#define LIBPNG_SOLIBRARY_SPEC "libpng.dylib"
#define LIBGIF_SOLIBRARY_SPEC "libungif.dylib"
#define LIBJPG_SOLIBRARY_SPEC "libjpeg.dylib"
#define LIBTIFF_SOLIBRARY_SPEC "libtiff.dylib"
#define LIBXPM_SOLIBRARY_SPEC "libXpm.dylib"
...
#define LIBPNG_SOLIBRARY_SPEC "libpng.dll"
#define LIBGIF_SOLIBRARY_SPEC "libungif.dll"
#define LIBJPG_SOLIBRARY_SPEC "jpeg-62.dll"
#define LIBTIFF_SOLIBRARY_SPEC "libtiff.dll"
#define LIBXPM_SOLIBRARY_SPEC "libXpm.dll" // Unlikely to exist.
#else
#define LIBPNG_SOLIBRARY "libpng.so"
#define LIBGIF_SOLIBRARY "libungif.so"
#define LIBJPG_SOLIBRARY "libjpeg.so"
#define LIBTIFF_SOLIBRARY "libtiff.so"
#define LIBXPM_SOLIBRARY "libXpm.so"
#define LIBPNG_SOLIBRARY_SPEC "libpng.so"
#define LIBGIF_SOLIBRARY_SPEC "libungif.so"
#define LIBJPG_SOLIBRARY_SPEC "libjpeg.so"
#define LIBTIFF_SOLIBRARY_SPEC "libtiff.so"
#define LIBXPM_SOLIBRARY_SPEC "libXpm.so"
#endif
#endif
...
int image_info::write(const char *filename, int flags) const {
  char *suffix;
...
void image_info::writejpg(int fildes, int quality) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writejpg\n", fildes);
    throw ImageInfoWriteJPGExc();
  }
  writejpg(fp);
}

void image_info::writewbmp(int fildes) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writewbmp\n", fildes);
    throw ImageInfoWriteWBMPExc();
  }
  writewbmp(fp);
}

void image_info::writepng(int fildes) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writepng\n", fildes);
...
void image_info::writebmp(int fildes) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writebmp\n", fildes);
    throw ImageInfoWriteBMPExc();
  }
  writebmp(fp);
}

void image_info::writeyuv(int fildes) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writeyuv\n", fildes);
    throw ImageInfoWriteYUVExc();
  }
  writeyuv(fp);
}

void image_info::writeppm(int fildes) const {
  FILE *fp;
  if ((fp = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descripter %d in writeppm\n", fildes);
...
void image_info::writeppm(FILE *fp) const {
...
void image_info::writeyuv(FILE *fp) const {
...
void image_info::convert_cmyk(){
  if(colourspace_type==IMAGEINFO_CMYK)
    return;
  convert_rgb();
  ...
  inverted_iinfo.pixels = new unsigned char[width*height*IMAGEINFO_CMYK_SIZE];
  inverted_iinfo.colourspace = IMAGEINFO_CMYK_SIZE;

  int ii = 0;
  for(int i=0;i<width*height*IMAGEINFO_RGB_SIZE;i+=3){ 
    double red   = pixels[i]/256.0;
    double green = pixels[i+1]/256.0;
    double blue  = pixels[i+2]/256.0;
    double black = min(1.0-red,1.0-green);
    black        = min(black,1.0-blue);
    double cyan  = 0.0;
    double magenta = 0.0;
    double yellow  = 0.0;
    if(fabs(black-1.0)>1e-5){
     cyan  = (1.0-red-black)/(1.0-black); 
     magenta = (1.0-green-black)/(1.0-black);
     yellow  = (1.0-blue-black)/(1.0-black);
    }

    //if(green>0.8&&red<0.1&&blue<0.1){
      //std::cout << "rgb:  " << red << " " << green << " " << blue <<"\n";
      //std::cout << "cmyk: " << cyan << " " << magenta << " " << yellow << " " << black <<"\n";
    //}
    inverted_iinfo.pixels[ii++] = (unsigned char)(cyan*255);
    inverted_iinfo.pixels[ii++] = (unsigned char)(magenta*255);
    inverted_iinfo.pixels[ii++] = (unsigned char)(yellow*255);
    inverted_iinfo.pixels[ii++] = (unsigned char)(black*255);
  }

  colourspace = IMAGEINFO_CMYK_SIZE;
  colourspace_type = IMAGEINFO_CMYK;
...
void image_info::convert_rgba(){
  ...
  }else if(colourspace_type==IMAGEINFO_ARGB){
    int ii = 0;
    for(int i=0;i<width*height*IMAGEINFO_ARGB_SIZE;i+=4){
      inverted_iinfo.pixels[ii++] = pixels[i+1];
      inverted_iinfo.pixels[ii++] = pixels[i+2];
      inverted_iinfo.pixels[ii++] = pixels[i+3];
      inverted_iinfo.pixels[ii++] = pixels[i];
    }
  }else{
    convert_rgb();
...
void image_info::convert_rgb(){
  ...
  if(colourspace_type==IMAGEINFO_ARGB){
    int ii = 0;
    for(int i=0;i<width*height*colourspace;i++){
      if(i%4!=0){
        inverted_iinfo.pixels[ii] = pixels[i];
...
image_info const& image_info::operator=(image_info const &iinfo){
  ...
  cmap = 0;
  ncmapped_colours = 0;
    copy(iinfo.get_width(),iinfo.get_height(),iinfo.get_pixels(),iinfo.get_colourspace_type(),iinfo.get_cmapped_pixels(),iinfo.get_cmap(),iinfo.get_ncolours());
  }
  return *this;
}
...
image_info::image_info(int width_in, int height_in, unsigned char *pixels_in, int colourspace_type_in){
  pixels = 0;
  cmapped_pixels = 0; /* This is somewhat bogus. */
  cmap = 0;
  ncmapped_colours = 0;
  copy(width_in, height_in, pixels_in, colourspace_type_in,get_cmapped_pixels(),get_cmap(),get_ncolours());
}

void image_info::copy(int width_in, int height_in, unsigned char *pixels_in, int colourspace_type_in, unsigned char *cmapped_pixels_in, unsigned char *cmap_in, int ncmapped_colours_in){
  ...
  if(colourspace_type==IMAGEINFO_MONOA)
    colourspace = IMAGEINFO_MONOA_SIZE;
  if(colourspace_type==IMAGEINFO_ARGB)
    colourspace = IMAGEINFO_ARGB_SIZE;

  if(pixels)
    delete [] pixels;
  if(cmapped_pixels)
    delete [] cmapped_pixels;
  if(cmap)
    delete [] cmap;
  pixels = new unsigned char[width*height*colourspace];
  memcpy(pixels,pixels_in,width*height*colourspace);
  if(cmapped_pixels_in&&cmap_in&&ncmapped_colours_in){
    cmapped_pixels = new unsigned char[width*height];
    memcpy(cmapped_pixels,cmapped_pixels_in,width*height);
    ncmapped_colours = ncmapped_colours_in;
    cmap = new unsigned char[ncmapped_colours*3];
    memcpy(cmap,cmap_in,ncmapped_colours*3);
  }
...
void image_info::writebmp(FILE *fp) const {
  int16_t type = 19778;
  try {
...
void image_info::readgif(const char *filename){
  ...
  if(!have_gif){
    ...
    else if(init_gif(LIBGIF_SOLIBRARY_SPEC)){
      have_gif = 1;
    }
  }
  if(!have_gif){
  ...
  cmap = new unsigned char[giffile->SColorMap->ColorCount*3];
  for(int icol=0;icol<giffile->SColorMap->ColorCount;icol++){
    cmap[icol*3] = ColorMap->Colors[icol].Red;
    cmap[icol*3+1] = ColorMap->Colors[icol].Green;
    cmap[icol*3+2] = ColorMap->Colors[icol].Blue;
  }
  ncmapped_colours = giffile->SColorMap->ColorCount;
	   ...
	   for (int i = 0; i < 4; i++) {
		 ...
		 throw ImageInfoReadGIFExc();
	           ...
	           pixels[(irow*width*colourspace)+(j*colourspace)+2] = ColorMap->Colors[gifpix[j]].Blue;
                   cmapped_pixels[irow*width+j] = gifpix[j];
	           if(colourspace_type==IMAGEINFO_RGBA){
          ...
          for(int i=0;i<height;i++){
	       ...
	       throw ImageInfoReadGIFExc();
	         ...
	         pixels[(i*width*colourspace)+(j*colourspace)+2] = ColorMap->Colors[gifpix[j]].Blue;
                 cmapped_pixels[i*width+j] = gifpix[j];
	         if(colourspace_type==IMAGEINFO_RGBA){
		   if(trans_index == gifpix[j])
...
int ColourQuantize(int width, int height, const unsigned char *pixels, unsigned int ncolours,
                   unsigned char *&output_pixels, std::vector<unsigned char> &output_colourmap);
...
void image_info::writegif(const char *filename) const {
  ...
  //EGifSetGifVersion("87a"); Why does this sometimes cause crash ?

  int ColorMapSize = 256;
  ColorMapObject *OutputColorMap = NULL;
  OutputColorMap = MakeMapObject(ColorMapSize, 0);

  /*
  GifByteType *RedBuffer = NULL, *GreenBuffer = NULL, *BlueBuffer = NULL, *OutputBuffer = NULL;;
  RedBuffer = new GifByteType[width*height];
  GreenBuffer = new GifByteType[width*height];
  BlueBuffer = new GifByteType[width*height];
  OutputBuffer = new GifByteType[width*height];
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
      RedBuffer[i*width+j] = tmp.pixels[i*width*tmp.colourspace+j*tmp.colourspace];
      GreenBuffer[i*width+j] = tmp.pixels[i*width*tmp.colourspace+j*tmp.colourspace+1];
      BlueBuffer[i*width+j] = tmp.pixels[i*width*tmp.colourspace+j*tmp.colourspace+2];
    }
  }

  std::cout << "Calling quantize buffer\n";
  if (QuantizeBuffer(width, height, &ColorMapSize,
		       RedBuffer, GreenBuffer, BlueBuffer,
		       OutputBuffer, OutputColorMap->Colors) == GIF_ERROR){
    std::cout << "Error quantizing buffers\n";
    return;
  }
  std::cout << "Called quantize buffer\n";
 
  delete [] RedBuffer;
  delete [] GreenBuffer;
  delete [] BlueBuffer;
  std::cout << "Deleted buffers\n";
  */

  unsigned char *output_pixels=0;
  std::vector<unsigned char> colourmap;
  ColourQuantize(tmp.get_width(), tmp.get_height(), tmp.get_pixels(), 256, output_pixels, colourmap);

  for(unsigned ii=0;ii<colourmap.size();ii+=3){
    OutputColorMap->Colors[ii/3].Red = colourmap[ii];
    OutputColorMap->Colors[ii/3].Green = colourmap[ii+1];
    OutputColorMap->Colors[ii/3].Blue = colourmap[ii+2];
  }

  err = EGifPutScreenDesc(giffile,width,height,8,0,OutputColorMap);
  err = EGifPutImageDesc(giffile, 0, 0, width, height, FALSE, NULL);

  GifByteType *Ptr = output_pixels;
  for (int i = 0; i < height; i++) {
    if (EGifPutLine(giffile, Ptr, width) == GIF_ERROR)
      { std::cerr << "Error putting line\n"; return; }
...
void image_info::writejpg(const char *filename, int quality) const {
  ...
  writejpg(outfile,quality);
}

void image_info::writejpg(FILE *outfile, int quality) const {
#ifdef USE_LIBJPEG
#ifdef _USE_DL_
  if(!have_jpeg){
    ...
    else if(init_jpeg(LIBJPG_SOLIBRARY_SPEC)){ 
      have_jpeg = 1;
    }
  }
  if(!have_jpeg){
...
void image_info::readtif(const char *filename){
  ...
  if(!have_tiff){
    ...
    else if(init_tiff(LIBTIFF_SOLIBRARY_SPEC)){ 
      have_tiff = 1;
    }
  }
  if(!have_tiff){
...
#ifdef USE_LIBTIFF
void _writetif(TIFF* tif, int flags, const image_info &tmp) {
#ifdef _USE_DL_
  if(!have_tiff){
    if(init_tiff(LIBTIFF_SOLIBRARY)){ 
      have_tiff = 1;
...
#endif 
  TIFFSetField(tif, TIFFTAG_IMAGEWIDTH, tmp.get_width());
  TIFFSetField(tif, TIFFTAG_IMAGELENGTH, tmp.get_height());
  ...
  uint32 *raster = (uint32*) _TIFFmalloc(tmp.get_width()*tmp.get_height()*sizeof(uint32));

  // We put in dpi fields which keep some people happy. Well, if only we could
  // convince them that there is no such thing as a "300 dpi" image ...
  // 7 is approx width of A4 in inches, so that 1800 pixels is approx. A4, 300dpi.
  float dpi = float(floor(tmp.get_width() / 7.0));
  TIFFSetField(tif, TIFFTAG_XRESOLUTION, dpi);
  TIFFSetField(tif, TIFFTAG_YRESOLUTION, dpi);

  const unsigned char *pixels = tmp.get_pixels();
  int ii=0;
  bool littleendian = CheckIfLittleEndian();
  for(int i=0;i<tmp.get_height();i++){
    for(int j=0;j<tmp.get_width();j++){
       ...
       raster[i*tmp.get_width()+j] = pix;
    }
  }
  for (int row = 0; row < tmp.get_height(); row++)
    TIFFWriteScanline(tif, raster+row*tmp.get_width(), row, 0);
  _TIFFfree(raster);
  TIFFClose(tif);
}
#endif  

void image_info::writetif(const char *filename, int flags) const {
  /* flags <100 implies CMYK, I am abusing the quality flag. */
#ifdef USE_LIBTIFF
#ifdef _USE_DL_
  if(!have_tiff){
    if(init_tiff(LIBTIFF_SOLIBRARY)){ 
...
void image_info::writetif(int fildes, const char *filename, int flags) const {
  /* flags <100 implies CMYK, I am abusing the quality flag. */
#ifdef USE_LIBTIFF
#ifdef _USE_DL_
  if(!have_tiff){
...
#endif  
  TIFF* tif = TIFFFdOpen(fildes, filename, "w");
  image_info tmp = *this;
  tmp.convert_rgba();
  if(flags<100){
    tmp.convert_cmyk();
...
void image_info::readpng(const char *filename){
  ...
  if(!have_png){
    ...
    else if(init_png(LIBPNG_SOLIBRARY_SPEC)){
      have_png = 1;
    }
  }
  if(!have_png){
...
void image_info::writewbmp(FILE *fp) const {
  WAPBMPHEADER wbh;
  wbh.TypeField = 0;
...
void image_info::writepng(FILE *outfile) const {
#ifdef USE_LIBPNG
#ifdef _USE_DL_
  if(!have_png){
    if(init_png(LIBPNG_SOLIBRARY)){
  ...
  // We put in dpi fields which keep some people happy. Well, if only we could
  // convince them that there is no such thing as a "300 dpi" image ...
  // 0.2 is approx width of A4 in meters, so that 1800 pixels is approx. A4, 300dpi.
  png_uint_32 dpm = (png_uint_32)(float(floor(width / 0.2)));
  info_ptr->x_pixels_per_unit = dpm;
  info_ptr->y_pixels_per_unit = dpm;
...
std::vector<std::string> image_info::GetSupportedReadFormats(){
  ...
  if(!have_png){
    if(init_png(LIBPNG_SOLIBRARY)){
      ...
      have_png = 1;
    } else fprintf (stderr, "Possibly no png capability\n");
  }
  if(!have_jpeg){
    if(init_jpeg(LIBJPG_SOLIBRARY)){
      have_jpeg = 1;
      ...
      have_jpeg = 1;
    } else fprintf (stderr, "Possibly no jpeg capability\n");
  }
  if(!have_gif){
    if(init_gif(LIBGIF_SOLIBRARY)){
      have_gif = 1;
      ...
      have_gif = 1;
    } else fprintf (stderr, "Possibly no gif capability\n");
  }
  if(!have_tiff){
    if(init_tiff(LIBTIFF_SOLIBRARY)){
      have_tiff = 1;
      ...
      have_tiff = 1;
    } else fprintf (stderr, "Possibly no tiff capability\n");
  }
...
void image_info::Dither(int mode){
  ...
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
       ...
       if(j+direction<width&&j+direction>=0){
         if(((int)pixels[i*width+j+direction] + (int)(alpha*e*255))<=255&&((int)pixels[i*width+j+direction] + (int)(alpha*e*255))>=0)
            pixels[i*width+j+direction] += (unsigned char)(alpha*e*255);
       }
       if(i<height-1&&j+direction<width&&j+direction>=0){
         if(((int)pixels[(i+1)*width+j-direction] + (int)(delta*e*255))<=255&&((int)pixels[(i+1)*width+j-direction] + (int)(delta*e*255))>=0)
           pixels[(i+1)*width+j-direction] += (unsigned char)(delta*e*255);
       }
       if(i<height-1){
         if(((int)pixels[(i+1)*width+j] + (int)(gamma*e*255))<=255&&((int)pixels[(i+1)*width+j] + (int)(gamma*e*255))>=0)
           pixels[(i+1)*width+j] += (unsigned char)(gamma*e*255);
       }
       if(i<height-1&&j+direction<width&&j+direction>=0){
         if(((int)pixels[(i+1)*width+j+direction] + (int)(beta*e*255))<=255&&((int)pixels[(i+1)*width+j+direction] + (int)(beta*e*255))>=0)
           pixels[(i+1)*width+j+direction] += (unsigned char)(beta*e*255);
       }
    }
    //direction = -direction;
  }
  /*
  Then do this for monchrome dither.
  */
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
...
void image_info::writexbm(const char *filename) const {
  ...
  writexbm(outfile,prefix);
  delete prefix;
}

void image_info::writexbm(int fildes, const char *prefix) const {
  FILE * outfile;
  if ((outfile = fdopen(fildes, "wb")) == NULL) {
    fprintf(stderr, "can't use file descriptor %d in writexbm\n", fildes);
    throw ImageInfoWriteXBMExc();
  }
  writexbm(outfile,prefix);
}
...
void image_info::writexbm(FILE *outfile, const char *prefix) const {
  ...
  for(int i=0;i<height;i++){
    ...
    bw_pixels[i*(width+7)/8+ width/8] = 0;
...
void image_info::readxpm(const char *filename){
  ...
  if(!have_xpm){
    ...
    else if(init_xpm(LIBXPM_SOLIBRARY_SPEC)){
      have_xpm = 1;
    }
  }
  if(!have_xpm){
