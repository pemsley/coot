...
double CXXCreator::fractionOfGridPointCoveredByAtom(CXXCoord theGridPoint, CXXCoord xGridVector, CXXCoord yGridVector, CXXCoord zGridVector, int atomNr) {
	...
	CXXCoord subGridPoint;
	CXXCoord distanceVector;
	...
	CXXCoord lowerBackLeftSubgridCorner = theGridPoint - xGridVector.scalarMultiply(0.5)
	...
	CXXCoord xStepVector = xGridVector.scalarMultiply(scale);
	CXXCoord yStepVector = yGridVector.scalarMultiply(scale);
	CXXCoord zStepVector = zGridVector.scalarMultiply(scale);
	
	// start at the CENTER of the first subcube ...
	CXXCoord centerOfFirstSubcube = lowerBackLeftSubgridCorner + xStepVector.scalarMultiply(0.5)
...
int CXXCreator::introduceMatter(double dielectricOfSolvent, double dielectricOfProtein) {
	...
	try {
		CXXCoord xGridVector = space->getSpaceSpanningVectorX();
		CXXCoord yGridVector = space->getSpaceSpanningVectorY();
		CXXCoord zGridVector = space->getSpaceSpanningVectorZ();
		
		
		CXXCoord gridOrigin = space->getOrigin();// grid origin is choosen so that all grid indices are positive ...
			
			// loop over all atoms and consider atoms with non zero charge
			
			double theAtomCharge;
			double theAtomRadius;
			CXXCoord theAtomCoord;
...
int CXXCreator::addAtomSolvationParameters() { // WARNING - need this to check if the solvent map does yet excist...
	...
	// get currentAtom solvation parameter
	// get currentAtom location
	CXXCoord theAtomCoord;
	double theAtomRadius;
	
	for (int atomNr = 0; atomNr < nSelAtoms; atomNr++) {
		
		theName = getAtomName(atomNr);
		...
		// make atom box around the atom -> iMin, iMax, jMin, jMax, kMin, kMax
		// loop over atom box just created
		// for all points corresponding to non zero points in the solvent map close to the current atom adjust the 
		// solvation parameter in the solvation map
		
		// if the point with starting value (0) give the point the point associated with the current atom
		// otherwise give the average of what the point is and what the current atom is
...
+   gridSpacing*gridSpacing*dielectric*K(i,j,k)*K(i,j,k)*potentialGrid(i,j,k)
	...
	double largestChange = 100;
	double largestPotential = 0;
	...
	try {
		...
		while (fabs(fractionalCorrection) > convergenceCriterion && n < maxIterations) { // while potential is still changeing...
			...
			for (int i = 0; i < space->getDimI(); i++) {
				for (int j = 0; j < space->getDimJ(); j++) {
					for (int k = 0; k < space->getDimK(); k++) {
						
						// odd/even ordering - only update when required ..
						if (((i+j+k)%2) == (n%2)) {
							
							// look up old potential at this grid point
							currentPotential = space->getPotential(i,j,k);			
							...
							eFactor = space->getEpsilonKappaSq(i,j,k);
							
							// eFactor*currentPotential = (sum(6 nearest Epsilons) + kappaFactor) sourceTerms: charge*chargeFactor
							resid = eFactor*currentPotential - space->getGridCharge(i,j,k)*chargeFactor
								- space->getDielGrid(i-1,j,k,0)*space->getPotential(i-1,j,k) - space->getDielGrid(i,j,k,0)*space->getPotential(i+1,j,k)
								- space->getDielGrid(i,j-1,k,1)*space->getPotential(i,j-1,k) - space->getDielGrid(i,j,k,1)*space->getPotential(i,j+1,k)
								- space->getDielGrid(i,j,k-1,2)*space->getPotential(i,j,k-1) - space->getDielGrid(i,j,k,2)*space->getPotential(i,j,k+1);
							
							// use residual and over correct current potenttial by over relaxation to calculate new potential
							newPotential  = currentPotential - w*resid/eFactor;
							
							// update the potential at the current grid point
							space->setPotential(i,j,k,newPotential);
...
clipper::NXmap<double> CXXCreator::coerceToClipperMap(clipper::Cell &cell){
	...
	CXXCoord doubleOrigin = space->getOrigin();
