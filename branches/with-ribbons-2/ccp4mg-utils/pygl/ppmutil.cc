...
image_info::~image_info(){
  ...
  //printf("image_info::~image_info()\n"); fflush(stdout);
  //printf("%d %d\n",get_width(),get_height()); fflush(stdout);
  if(pixels)
    delete [] pixels;
...
void image_info::readbmp(const char *filename)
{
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&offset,sizeof(int32_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&width, sizeof(int));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&height, sizeof(int));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&nplanes, sizeof(int16_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&bpp, sizeof(int16_t));
    }
    catch (...) {
      fprintf(stderr,"Error getting nplanes\n");
    }
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&comp, sizeof(int32_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&bitmap_size, sizeof(int32_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&horiz_res, sizeof(int32_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&vert_res, sizeof(int32_t));
    }
    catch (...) {
    ...
    try {
      ...
      if(bigEndian) make_little_endian(&colours_used, sizeof(int32_t));
    }
    catch (...) {
...
void image_info::readpng(const char *filename){
  ...
  for (int i=0; i<height; i++){
    if(color_type==PNG_COLOR_TYPE_PALETTE){
      ...
      int doTrans = png_get_tRNS(png_ptr, info_ptr, &trans, &num_trans, &trans_values);
      for (int j=0; j<width; j++){
        ...
        if(doTrans){
         pixels[i*width*colourspace+j*colourspace+3] = trans[row_pointers[i][j]];
        }else
         pixels[i*width*colourspace+j*colourspace+3] = 255;
      }
    }else{
...
void image_info::writepng(FILE *outfile) const {
  ...
  png_set_pHYs(png_ptr, info_ptr, dpm, dpm, PNG_RESOLUTION_METER);
  ...
  for (int i=0; i<height; i++){
    row_pointers[i]=(png_bytep)png_malloc(png_ptr, colourspace*width*sizeof(unsigned char));
    memcpy(row_pointers[i],pixels+i*width*colourspace,sizeof(unsigned char)*colourspace*width);
