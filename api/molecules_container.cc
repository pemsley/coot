...
molecules_container_t::read_standard_residues() {
   ...
   if (! env_var_filename) {
      ...
      if (status != 0) { // standard-residues file was not found in
         ...
         atom_selection_container_t t_asc = get_atom_selection(standard_file_name, "", false, true, false, false);
      ...
      standard_residues_asc = get_atom_selection(env_var_filename, "", false, true, false, false);
   }
...
molecules_container_t::read_pdb(const std::string &file_name) {
   ...
   int status = -1;
   atom_selection_container_t asc = get_atom_selection(file_name, "", false, true, false, false);
   if (asc.read_success) {
