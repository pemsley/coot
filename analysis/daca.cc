...
coot::daca::fill_reference_fragments() {
   ...
   if (file_exists(fn)) {
      atom_selection_container_t asc = get_atom_selection(fn, "", false, false, false, false);
      if (asc.read_success) {
...
coot::daca::write_tables_using_reference_structures_from_dir(const std::string &dir_name,
   ...
   for (unsigned int i=0; i<files.size(); i++) {
      ...
      atom_selection_container_t asc = get_atom_selection(fn, "", false, true, false, false);
      if (asc.read_success) {
...
coot::daca::score_molecule(const std::string &pdb_file_name) {
   ...
   if (coot::file_exists(pdb_file_name)) {
      atom_selection_container_t asc = get_atom_selection(pdb_file_name, "", false, false, false, false);
      if (asc.read_success) {
         mmdb::Model *model_p = asc.mol->GetModel(1);
         if (model_p) {
