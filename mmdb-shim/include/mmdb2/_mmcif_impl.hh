   ...
   namespace mmcif {
      ...
      public:
         ...
         void PutReal(realtype R, cpstr T, int nrow, int prec = 8) {
            char b[64];
            std::snprintf(b, sizeof b, "%.*f", prec, (double)R);
            wput(T, nrow, b);
         }
      ...
      public:
         ...
         int ReadMMCIFData(cpstr fname) {
            try {
               owned_doc.reset(new gemmi::cif::Document(gemmi::cif::read_file(fname ? fname : "")));
         ...
         int GetReal(realtype &R, cpstr CName, cpstr TName, bool /*Remove*/ = false) {
            ...
            try {
               R = std::stod(gemmi::cif::as_string(*v));
            } catch (...) {
               return CIFRC_WrongFormat;
            }
            return CIFRC_Ok;
         }
         int GetInteger(int &I, cpstr CName, cpstr TName, bool /*Remove*/ = false) {
            ...
            if (gemmi::cif::is_null(*v)) return CIFRC_NoData;
            try {
               I = std::stoi(gemmi::cif::as_string(*v));
            } catch (...) {
               return CIFRC_WrongFormat;
            }
            return CIFRC_Ok;
         ...
         void flush() {
            for (auto &l : loops)
               if (l.write_mode) l.flush(blk());
            for (auto &s : structs)
               if (s.write_mode) s.flush(blk());
         }
      };
      ...
      inline pstr Loop::GetString(cpstr TName, int nrow, int &RC) {
         ...
         if (nrow < 0 || (size_t)nrow >= g->length()) {
            RC = CIFRC_WrongIndex;
            return nullptr;
         }
      ...
      inline int Loop::GetReal(realtype &R, cpstr TName, int nrow, bool /*Remove*/) {
         ...
         try {
            R = std::stod(gemmi::cif::as_string(raw));
         } catch (...) {
            return CIFRC_WrongFormat;
         }
         return CIFRC_Ok;
      ...
      inline int Loop::GetInteger(int &I, cpstr TName, int nrow, bool /*Remove*/) {
         ...
         if (gemmi::cif::is_null(raw)) return CIFRC_NoData;
         try {
            I = std::stoi(gemmi::cif::as_string(raw));
         } catch (...) {
            return CIFRC_WrongFormat;
         }
         return CIFRC_Ok;
      ...
      inline pstr Loop::GetTag(int tagNo) {
         gemmi::cif::Loop *g = gloop();
         if (!g) {
            if (tagNo < 0 || (size_t)tagNo >= wtags.size()) return nullptr;
            sret.push_back(wtags[tagNo]);
            return (pstr)sret.back().c_str();
         }
      ...
      public:
         ...
         int ReadMMCIFFile(cpstr FName, int /*flags*/ = 0) {
            try {
               doc = gemmi::cif::read_file(FName ? FName : "");
            } catch (const std::exception &) {
               return CIFRC_CantOpenFile;
            }
            rebuild();
      ...
      inline pstr GetCIFMessage(pstr buffer, int rc) {
         ...
         switch (rc) {
            ...
            case CIFRC_NoField:
               m = "field not found";
               break;
            case CIFRC_WrongFormat:
               m = "wrong value format";
               break;
            case CIFRC_WrongIndex:
               m = "row index out of range";
               break;
            case CIFRC_NotALoop:
               m = "category is not a loop";
               break;
            case CIFRC_NotAStructure:
               m = "category is not a structure";
               break;
            case CIFRC_NoData:
               m = "no data";
               break;
            case CIFRC_CantOpenFile:
               m = "cannot open file";
               break;
            case CIFRC_NoDataLine:
               m = "no data_ line";
               break;
            case CIFRC_Created:
               m = "category created";
