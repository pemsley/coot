...
namespace mmdb {
   namespace math {
      ...
      enum GRAPH_BOND { BOND_SINGLE = 1,
                        BOND_DOUBLE = 2,
                        BOND_AROMATIC = 3,
                        BOND_TRIPLE = 4 };
      enum GRAPH_RC { MKGRAPH_Ok = 0,
                      MKGRAPH_NoAtoms = -1,
                      MKGRAPH_ChangedAltLoc = 1,
                      MKGRAPH_MaxOccupancy = 2 };
      enum GRAPH_MATCH_FLAG { GMF_UniqueMatch = 0x00000001,
                              GMF_NoCombinations = 0x00000002 };
      enum VERTEX_EXT_TYPE { EXTTYPE_Ignore = 0,
                             EXTTYPE_Equal = 1,
                             EXTTYPE_AND = 2,
                             EXTTYPE_OR = 3,
                             EXTTYPE_XOR = 4,
      ...
      public:
         ...
         Vertex(cpstr chem_elem, cpstr vname) {
            SetVertex(chem_elem);
            name = vname ? vname : "";
         }
      ...
      public:
         ...
         Edge() {}
         Edge(int vx1, int vx2, int btype) {
            v1 = vx1;
            v2 = vx2;
            type = btype;
         }
         Edge(int vx1, int vx2, cpstr btype) {
            v1 = vx1;
            v2 = vx2;
            type = gdetail::bond_from_string(btype);
         }

         void SetEdge(int vx1, int vx2, int btype) {
            v1 = vx1;
            v2 = vx2;
            type = btype;
         }
      ...
      public:
         ...
         void GetVertices(PPVertex &v, int &n) {
            v = V.data();
            n = (int)V.size();
         }
         void GetEdges(PPEdge &e, int &n) {
            e = E.data();
            n = (int)E.size();
         }
      ...
      public:
         ...
         void SetMaxNofMatches(int m, bool stopOnMaxN) {
            maxNofMatches = m > 0 ? m : 1;
            stopOnMax = stopOnMaxN;
         }
      ...
      inline void GraphMatch::MatchGraphs(PGraph g1, PGraph g2, int minMatch,
         ...
         std::function<void(int)> rec = [&](int idx) {
            ...
            if (idx > n1) {
               ...
               if (sz >= minMatch && sz >= maxMatch) {
                  ...
                  if ((int)best.size() < maxNofMatches)
                     best.push_back(cur);
                  else if (stopOnMax)
                     Stop = true;
               }
         ...
         for (auto &m : best) {
            Match mm;
            for (auto &pr : m) {
               mm.f1.push_back(pr.first);
               mm.f2.push_back(pr.second);
            }
      ...
      inline void GraphMatch::GetMatch(int MatchNo, ivector &FV1, ivector &FV2,
         ...
         if (MatchNo < 0 || MatchNo >= (int)matches.size()) {
            FV1 = FV2 = nullptr;
            nv = 0;
            p1 = p2 = 0;
            return;
         }
         if ((int)fv1.size() != (int)matches.size()) {
            fv1.resize(matches.size());
            fv2.resize(matches.size());
         }
         Match &m = matches[MatchNo];
         nv = (int)m.f1.size();
         std::vector<int> &a = fv1[MatchNo];
         std::vector<int> &b = fv2[MatchNo];
         a.assign(nv + 1, 0);
         b.assign(nv + 1, 0);  // 1-indexed: [1..nv] valid
         for (int i = 0; i < nv; ++i) {
            a[i + 1] = m.f1[i];
            b[i + 1] = m.f2[i];
         }
