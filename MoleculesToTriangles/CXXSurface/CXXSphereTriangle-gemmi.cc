/*
 * MoleculesToTriangles/CXXSurface/CXXSphereTriangle-gemmi.cc
 *
 * gemmi-native twin of CXXSphereTriangle.cpp.
 *
...
CXXSphereTriangle::CXXSphereTriangle() :
   theRadius(0), theCentre(CXXCoord<CXXCoord_ftype>(0.,0.,0.)) {}
...
CXXSphereTriangle::CXXSphereTriangle(CXXSphereElement *se, size_t *inputVertices, size_t *inputEdges,
                                     double inRad, CXXCoord<CXXCoord_ftype>&incent, const gemmi::Atom* anAtom) {
   theAtom = anAtom;
   theSphereElement = se;
   for (int i=0; i<3; i++) {
...
size_t CXXSphereTriangle::vertex(size_t i) const { return triangleVertices[i]; }
size_t CXXSphereTriangle::edge(size_t i) const { return triangleEdges[i]; }
double CXXSphereTriangle::radius() const { return theRadius; }
const CXXCoord<CXXCoord_ftype>& CXXSphereTriangle::centre() const { return theCentre; }
...
int CXXSphereTriangle::bisect(double radians) {
   ...
   if (longestLength>radians) {
      ...
      if (newNormal*(theSphereElement->vertex(triangleVertices[iLongest]).vertex()-theCentre)<0.)
         newNormal *= -1.;
      CXXSphereTriangleEdge newEdge2(newNormal, iNewVertex, vertex((iLongest+2)%3),
                                     theCentre, theCentre, theRadius, theSphereElement);
      newEdge2.setCircle(0);
      size_t iNewEdge2 = theSphereElement->addEdge(newEdge2);

      newNormal.scale(-1);
      CXXSphereTriangleEdge newEdge3(newNormal, vertex((iLongest+2)%3), iNewVertex,
                                     theCentre, theCentre, theRadius, theSphereElement);
...
int CXXSphereTriangle::setVertex(size_t i, size_t newOne) { triangleVertices[i] = newOne; return 0; }
CXXSphereElement *CXXSphereTriangle::sphereElement() const { return theSphereElement; }
int CXXSphereTriangle::setEdge(size_t i, size_t newOne) { triangleEdges[i] = newOne; return 0; }
int CXXSphereTriangle::setSphereElement(CXXSphereElement *se) { theSphereElement = se; return 0; }
int CXXSphereTriangle::setCentre(const CXXCoord<CXXCoord_ftype>&crd) { theCentre = crd; return 0; }
int CXXSphereTriangle::setRadius(const double rad) { theRadius = rad; return 0; }
const gemmi::Atom* CXXSphereTriangle::getAtom() const { return theAtom; }
int CXXSphereTriangle::setAtom(const gemmi::Atom* anAtom) { theAtom = anAtom; return 0; }
