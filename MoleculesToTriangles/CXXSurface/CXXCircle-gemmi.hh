/*
 * MoleculesToTriangles/CXXSurface/CXXCircle-gemmi.hh
 *
 * gemmi-native twin of CXXCircle.h. Atom token const gemmi::Atom*;
 * CXXNewHood/CXXBall used via pointer (forward-declared).
 *
 ...
 */
#ifndef CXXCircle_gemmi_included
#define CXXCircle_gemmi_included
...
namespace coot {
   namespace m2t {
      ...
      private:
         ...
         std::list<CXXCircleNode> theNodes;
         int nIntersectingCircles;
         std::vector<CXXCircleNode *> theStarts;
         std::vector<CXXCircleNode *> theStops;
         int completelyEaten;
         int nodeNumber;
         int containsEatenNodes;
      public:
         CXXCircle();
         CXXCircle(CXXNewHood *aHood, const gemmi::Atom* atom2, double radiusOfAtom2, double probeRadius);
         CXXCircle(CXXNewHood *aHood, const CXXBall &aBall);

         void performPrecalculations();
         int meetsCircle(const CXXCircle &otherCircle, std::vector<CXXCoord<CXXCoord_ftype> > &nodeList) const;
         ...
         const gemmi::Atom* getAtomJ() const;
         const std::list<CXXCircleNode> &getNodes() const { return theNodes; }
         const CXXBall *getBallJ() const { return theBallJ; }

         double getRadiusOfSphere() const;
         double getRadiusOfSecondSphere() const;
         double getRadiusOfCircle() const { return radiusOfCircle; }
         void setRadiusOfCircle(const double &value) { radiusOfCircle = value; }
         const CXXCoord<CXXCoord_ftype>& getCentreOfCircle() const { return centreOfCircle; }
         void setCentreOfCircle(const CXXCoord<CXXCoord_ftype>&aCentre) { centreOfCircle = aCentre; }

         const CXXCoord<CXXCoord_ftype>& getNormal() const;
         void setNormal(const CXXCoord<CXXCoord_ftype>&aNormal) { theNormal = aNormal; }
         void setCentreToCircle(const CXXCoord<CXXCoord_ftype>&aNormal) { centreToCircle = aNormal; }
         ...
         void setContainsEatenNodes(const int flag) { containsEatenNodes = flag; }
         int getContainsEatenNodes() const { return containsEatenNodes; }
         ...
         int getNIntersectingCircles() const { return nIntersectingCircles; }
         bool abBracketsC(const CXXCircleNode &nodea, const CXXCircleNode &nodeb, const CXXCircleNode &nodec) const;
         bool smallabBracketsC(const CXXCircleNode &nodea, const CXXCircleNode &nodeb, const CXXCircleNode &nodec) const;
      };
   }
}
