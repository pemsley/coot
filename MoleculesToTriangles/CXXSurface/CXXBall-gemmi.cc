/*
 * MoleculesToTriangles/CXXSurface/CXXBall-gemmi.cc
 *
 * gemmi-native twin of CXXBall.cpp.
 *
...
void CXXReentrantProbeBall::initSphereElement(CXXSphereElement &theSphere, const double &delta, const CXXSphereElement &unitCellAtOriginForDelta) const {
   theSphere.initWith(theCoord, theAtomI, theAtomJ, theAtomK, delta, theRadius, includeAtoms);
}
                          ...
                          std::map<const CXXBall*, std::vector<const CXXBall*> > &contactMap)
{
   ...
   for (std::vector<const CXXBall*>::iterator ball = balls.begin(); ball != ballsEnd; ++ball){
   ...
   for (std::vector<const CXXBall*>::iterator ball = contextBalls.begin(); ball != contextBallsEnd; ++ball){
       maxBallRadius = (maxBallRadius > (*ball)->getRadius() ? maxBallRadius : (*ball)->getRadius());
   }
   ...
   for (std::vector<const CXXBall*>::iterator ballIter = contextBalls.begin(); ballIter!=contextBallsEnd; ++ballIter){
       for (int i=0; i<3; i++){
           limits[i][0] = (limits[i][0]<(**ballIter)[i]?limits[i][0]:(**ballIter)[i]);
           limits[i][1] = (limits[i][1]>(**ballIter)[i]?limits[i][1]:(**ballIter)[i]);
       }
   }
   for (int i=0; i<3; i++){ limits[i][0] -= 0.0001; limits[i][1] += 0.0001; }
   ...
   for (int i=0; i<3; i++){
       ...
       if (nMinimumBins<maxNBins){ nBins[i] = nMinimumBins; binWidth[i] = minimumBinSize; }
       else { nBins[i] = maxNBins; binWidth[i] = (limits[i][1]-limits[i][0])/double(maxNBins); }
   }
   ...
   for (int i=0; i<nBins[0]; i++){
       ...
       for (int j=0; j<nBins[1]; j++){ binnedballs[i][j].resize(nBins[2]); }
   }
   ...
   for (std::vector<const CXXBall*>::iterator ballIter = balls.begin(); ballIter!=ballsEnd; ++ballIter){
       ...
       for (int searchBinX = startBinX; searchBinX<endBinX; searchBinX++){
           for (int searchBinY = startBinY; searchBinY<endBinY; searchBinY++){
               for (int searchBinZ = startBinZ; searchBinZ<endBinZ; searchBinZ++){
                   ...
                   for (std::vector<const CXXBall*>::const_iterator otherball = searchBin.begin(); otherball!=binSearchEnd; ++otherball){
