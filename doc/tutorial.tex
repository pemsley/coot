...
Typically one would launch coot from a terminal window, by typing:

\texttt{\$ coot}

But in the CCP4 Cloud, it's different - there one starts \emph{Coot}
...
%    \includegraphics[width=70mm]{images/Map-Screenshot.png}
    ...
    \caption{\emph{Coot} after reading an MTZ file and zoomed in.}
...
icon buttons (here ``Map'') displayed on the right side of the \emph{Coot} window.
...
\textsl{ [\emph{Coot} displays the Model/Fit/Refine window]}
...
\textsl{  [\emph{Coot} adds intermediate white atoms and pops up a dialog]}
...
\item Use the validation tools in \emph{Coot} to examine how good a PDB file
  this is... 
