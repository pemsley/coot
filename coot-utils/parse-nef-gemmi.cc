...
std::string format_to_string(nef::FileFormat fmt) {
   switch (fmt) {
...
bool is_null_value(const std::string& val) {
    return val == "." || val == "?" || val.empty();
}

std::optional<double> parse_double(const std::string& val) {
...
std::optional<int> parse_int(const std::string& val) {
    if (is_null_value(val)) return std::nullopt;
    try {
        return std::stoi(val);
    } catch (...) {
