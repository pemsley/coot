...
debug(0)
{
	...
	const CXXCoord yAxis(0.,1.,0.,0.);
	const CXXCoord zAxis(0.,0.,1.,0.);
	...
	for (int iOmega=0; iOmega<=nOmegaSteps; iOmega++){
		...
		for (int iTheta=0; iTheta<=nThetaSteps; iTheta++){
			...
			CXXCoord xyz(coordFromThetaOmega(theta, omega));
			aNode.setCoord(xyz);
	...
	for (int iOmega=0; iOmega<nOmegaSteps; iOmega++){
		for (int iTheta=0; iTheta<nThetaSteps; iTheta++){
			int iTriangle = flatTriangles.size();
...
int CXX_mot::CXXTorusElement::addNode(CXXTorusNode &aNode){
	...
	CXXCoord xyz = coordFromThetaOmega(newNode.getTheta(), newNode.getOmega());
...
int CXX_mot::CXXTorusElement::upload(CXX_mot::CXXSurface *aSurface){
	...
	int oldVertexCount;
	{
		...
		for (unsigned int i=0; i< nodes.size(); i++){
			...
			CXXCoord normal = normalToProbeAtTheta(accessible, nodes[i].getTheta());
			normal.scale(-1.);
...
// 		list<CXXTriangle *, CXX_old::CXXAlloc<CXXTriangle> >::iterator edgeTrianglesEnd = edgeTriangles.end();
		...
		if (triangleFound){
			...
			CXXCoord xyz(coordFromThetaOmega(theta2, omega));
			aNode.setCoord(xyz);
