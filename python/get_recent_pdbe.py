...
# FIXME: logic! What do we want to do? Either:
#   1.) thread download, refmac etc
#      a) deal afterwards with graphics, generated pdb/mtz
...
def pdbe_latest_releases_gui():
    ...
    class GetUrlThread(threading.Thread):
        ...
        def run(self):
            try:
                # FIXME: local file naem has to be empty I think, so remove first
                ...
                print "BL DEBUG:: urlinfo", url_info
                self.status = 0 #?
            except socket.timeout:
    ...
    def start_table():
        if thread.get_url_status() == 0:
            dig_table(get_recent_json(json_file_name), 0)
            return False  # stop
        return True  # continue
...
if __name__ == "__main__":
    ...
    def string_append_with_spaces(ls):
        ...
        if ls:
            ...
            return [""]
    def info_dialog(txt):
        print "INFO DIALOG::", txt
    def read_pdb(pdb):
        print "READ_PDB", pdb
    def mmcif_sfs_to_mtz(sfs_cif_file_name, sfs_mtz_file_name):
        print "MMCIF_SFS_TO_MTZ", sfs_cif_file_name, sfs_mtz_file_name
        return 0
    gtk.main()
