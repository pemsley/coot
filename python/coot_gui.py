...
def historyDown():
    ...
    def insert_normal_text(s):
        ...
        textbuffer.insert_with_tags(
            end, str(s), textbuffer.create_tag(foreground="black"))
        scroll_to_end()
    ...
    def warning_event(entry, event):
        if (event.state & gtk.gdk.CONTROL_MASK):
            ...
            tag1 = textbuffer.create_tag(
                foreground="red", weight=pango.WEIGHT_BOLD)
            insert_tag_text(
                tag1, "Don't type here.  Type in the white entry bar\n")
            insert_prompt()
    ...
    def do_function(widget, entry):
        ...
        try:
            ...
            if (entry_text != None):
                ...
                if guile_function:
                    insert_normal_text(
                        "BL INFO:: Detected guile scripting!\nYou should use python commands!!\nBut I'm a nice guy and translated it for you, this time...!\n")
                else:
                    insert_normal_text(
...
def make_difference_map_gui():
    ...
    def go_function_event(widget):
        ...
        active_mol_no_ref = get_option_menu_active_molecule(
            option_menu_ref_mol, coot_utils.map_molecule_list_ref)
        active_mol_no_sec = get_option_menu_active_molecule(
            option_menu_sec_mol, coot_utils.map_molecule_list_sec)
    ...
    coot_utils.map_molecule_list_ref = fill_option_menu_with_map_mol_options(
        option_menu_ref_mol)
    coot_utils.map_molecule_list_sec = fill_option_menu_with_map_mol_options(
        option_menu_sec_mol)
