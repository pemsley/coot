	...
	for ( i = 0; i <dim[0] ; i++) {
		for (j = 0; j <dim[1] ; j++) {
			for (k = 0; k <dim[2] ; k++) {
				float value;
				if (smoothFlag) {
					if (0&&(value = getDistanceAt3i(i, j, k)) > 0.){
					}
					else {
...
extern CSymbol int cAtomAt3fv (void *theMap, float xyz[3], float AtomRadius){
	return (((SolventMap *)theMap)->atomAt3fv(xyz, AtomRadius));
}
extern CSymbol int cAtomAt3f (void *theMap, float x, float y, float z, float AtomRadius){
	return (((SolventMap *)theMap)->atomAt3f(x, y, z, AtomRadius));
}
extern CSymbol int cAtomAt3dv (void *theMap, fftw_real xyz[3], fftw_real AtomRadius){
	return (((SolventMap *)theMap)->atomAt3dv(xyz, AtomRadius));
}
extern CSymbol int cAtomAt3d (void *theMap, fftw_real x, fftw_real y, fftw_real z, fftw_real AtomRadius){
