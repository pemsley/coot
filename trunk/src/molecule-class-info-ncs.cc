...
// 
int
molecule_class_info_t::make_dynamically_transformed_maps(int imol_map,
   ...
   //
   // We have a map and coordinates and they have inexact NCS.  We
   // already have coordinate ghosts, and this function provides
   // _maps_ that are rotated by ncs - but the mechanism is different
   // - coordinates ghosts are part of the coordinates molecule, but
   // map ghosts are simply newly created maps.
   //
   // The new maps are just copies of the input map, but have
   // RTop_orths associated with them (and a flag) so that they are
   // drawn by a different mechanism.
