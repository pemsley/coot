...
void main() {
  ...
  float specular_strength = 1.9; // I don't like specular/phong map lights at the moment.
  specular_strength = 0.9; // for now

  vec4 specular_light_colour = vec4(0.7, 0.7, 0.7, 1.0);
  vec3 lightdir = normalize(vec3(-2,-1, 5));
  float dp = dot(Normal, -lightdir);
  dp = max(dp, 0.0); // no negative dot products for diffuse for now
  ...
  float flat_frac = 0.3; // ambient scaling
  vec4 col_2 = line_colour_local * dp;
  // vec4 col_3 = mix(col_2, col_1, flat_frac) + specular;
  vec4 col_3 = col_2 + col_1 * flat_frac + specular;
  vec4 col_4 = mix(bg_col, col_3, f_2);
  // col_4.a = 0.03; // needs alpha in the GL context?
