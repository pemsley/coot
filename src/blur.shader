...
float get_weight_xy(float r_sqrd, float tightness) {
   ...
   // the weight that we want to give to the colour of this
   // pixel depends on the depth of this n_pixel
   // and the distance that this pixel is away from the
   // central pixel.

   float r = sqrt(r_sqrd); // between 0.0 and 1.0
   float t = clamp(tightness, 0.0, 1.0);
   float f_1 = 1.0 - 0.9 * t * r; // 0.2 needs tweaking
...
float get_weight_z_1(float depth_centre) {
    ...
    float w = 1.3 * abs(depth_centre - 0.231);
    return w;
...
float get_weight_z_2(float depth_this, float depth_centre) {
    ...
    float c = 0.652; // 0.66 looks good, but we
    ...
    if (depth_delta < c) w = 0.61205 + (c - depth_delta)/c;

    return w;

}

vec3 sampling_blur(const int n_pixels_max) {
    ...
    float tightness = ((1.0 - 0.231) - w_1)/(1.0 - 0.231); // tightness between 0 (close to rotation centre to 1.0 (at the back)
    vec3 r = vec3(0.0, 0.125, 0.0);
    // restore this to 0.1 to 0.3 when the blur is improved
    if (depth_centre >= 0.0 && depth_centre <= 0.231) { // the front (in screen z) doesn't get blurred
