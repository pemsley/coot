...
void main() {
   ...
   frag_pos_light_space_transfer = light_space_mvp * vec4(frag_pos_transfer, 1.0);
}
...
void main() {
   ...
   for (int i=0; i<2; i++) {
      ...
      if (light_sources[i].is_on) {
         ...
         sum_col += ambient + diffuse * shadow_diffuse + specular * shadow_specular;
         // sum_col += 0.3 * shadow_diffuse;
         // sum_col += vec4(0.2, 0.0, 0.0, 1.0);
