...
on_glyco_wta_fit_button_clicked(G_GNUC_UNUSED GtkButton       *button,
   ...
   if (is_valid_model_molecule(imol)) {
      ...
      if (is_valid_map_molecule(imol_map)) {
         std::string tt;
         if (t == "NAG-NANG-BMA")           tt = "NAG-NAG-BMA";
         ...
         coot::residue_spec_t as(coot::atom_spec_t(aa.second));
         // coot::cho::add_named_glyco_tree(tt, &g.molecules[imol].atom_sel, imol, xmap, g.Geom_p(), as.chain_id, as.res_no);
         // g.molecules[imol].add_named_glyco_tree(tt, g.Geom_p(), res_spec); // needs bonds update
      }
   }
   if (frame)
      gtk_widget_set_visible(frame, FALSE);
