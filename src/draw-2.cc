...
void init_central_cube_shaders() {
   ...
   shader_program_source sps = parse_shader("central-cube.shader");
   err = glGetError();
   std::cout << "init_central_cube_shaders() parse_shader() err is " << err << std::endl;
   unsigned int programID = CreateShader(sps.VertexSource, sps.FragmentSource);
   err = glGetError();
   std::cout << "init_central_cube_shaders() CreateShader() returned " << programID << " with err "
             << err << std::endl;
